...
*Reliability* is addressed through double-coding: a subset of the sample (approximately 20 percent of speeches) is independently coded by a trained research assistant working from the same codebook. Inter-coder reliability is calculated using Cohen's kappa (Cohen, 1960), with a target threshold of κ ≥ 0.70 — representing "substantial agreement" in the standard interpretive framework — regarded as acceptable for this type of categorical coding (Landis & Koch, 1977). Disagreements are resolved through discussion and codebook refinement. All disagreements and resolutions are documented in an audit trail.
