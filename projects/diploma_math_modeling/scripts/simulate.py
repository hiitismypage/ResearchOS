...
FC_RATE = 0.02   # доля мощности x̄_jf, идущая на постоянные издержки c_jf
CLIP_B_FACTOR = 5.0   # B(t) не превышает 5× B(0) для ненулевых элементов
CLIP_B_ABS    = 1.0   # потолок для нулевых элементов B(0), б/р
...
# Статистический вектор (C+G+NX) нарушает баланс X=AX+u при TOTAL-потоках МОБ:
# 33%-я невязка вследствие высокой открытости экономики (экспорт в обработке).
# Используем модельно-согласованный вектор u0 = max((I−A0)·X0, ε):
#   — гарантирует статический баланс на t=0
#   — обеспечивает x*(1) ≈ X0 при квазистатическом решении (ур-е 4.7)
_r0 = (np.eye(5) - A0) @ X0        # = X0 − A0·X0 — доступный конечный выпуск
U0  = np.maximum(_r0, 0.01)         # клипируем: отрицательные компоненты → 0
...
KAPPA = np.array([0.125, 0.050, 0.450, 0.180, 0.050])  # κ_j: норма реинвестирования
...
def leontief_target(x: np.ndarray, A: np.ndarray,
    ...
    Квазистатическая цель (ур-е 4.7):
        (I − A(t) − G_U·B(t)) · x*(t+1) = u(t)
    При ρ(A + G_U·B) < 1 матрица (I−A−G_U·B) невырождена → прямое решение.
    Численно устойчивее B⁺-поправки при несбалансированных начальных условиях.
    """
    M = np.eye(N) - A - G_U * B
    try:
...
def io_balance_residual(x: np.ndarray, x_next: np.ndarray,
    ...
    Относительная невязка динамического МОБ (ур-е 4.37):
        r ≈ x(t+1) − A(t+1)·x(t+1) − B(t+1)·(x(t+1) − x(t)) − u(t)
    Возвращает ‖r‖ / ‖x(t+1)‖.
    """
...
def run(seed: int | None = 42) -> pd.DataFrame:
    """
...
def make_figures(df: pd.DataFrame, out: Path) -> None:
    ...
    def _smooth(arr: np.ndarray, w: int = 5) -> np.ndarray:
        """Centered rolling mean; boundary values filled via edge extension."""
        kernel = np.ones(w) / w
        pad = w // 2
    ...
    for j in range(N):
        ax.plot(years, X_norm[:, j],
                color=COLORS[j], lw=0.8, alpha=0.25, zorder=1)
        ax.plot(years, _smooth(X_norm[:, j]),
                color=COLORS[j], lw=2.0, marker=MARKERS[j],
                markevery=5, label=SECTOR_NAMES[j], zorder=2)
    ...
    ax.plot(years, gdp_norm, color='#1f77b4', lw=0.8, alpha=0.25)
    ax.plot(years, _smooth(gdp_norm), color='#1f77b4', lw=2.5, ls='-',
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help='Зерно ГСЧ для воспроизводимости (None = случайное)'
    )
    parser.add_argument(
